	...
	pub async fn block_by_number(
		...
		let mut rich_block =
			match rich_block_or_none(block, statuses, block_hash, full, base_fee, false) {
				Some(rich_block) => rich_block,
				// Preserve original fix from bf47d4b49e94fcffbfe3896f4570577cdc40573d: if block data is missing but we have a
				// block number, synthesize an empty block rather than returning None.
				None => {
					if let BlockNumberOrHash::Num(block_number) = number_or_hash {
						...
						return Ok(Some(rich_block_build(
							eth_block,
							Default::default(),
							Some(eth_hash),
							full,
